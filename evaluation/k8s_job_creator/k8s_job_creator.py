...
class KubernetesJobCreator(object):
    ...
    def create_jobs(self, evaluation_jobs, parameter_sweep_file,
        ...
        for cfg_dict in cfg_gen.sweep_parameters(parameter_sweep_file):
            ...
            cfg_gen.write_config_file(base_config_lines, cfg_dict,
                                      config_local)
    ...
    def createJob(self, evaluation_job):
        job = client.V1Job(
            ...
            spec=client.V1JobSpec(
                template=self._create_pod_template_spec_for_job(
                    evaluation_job),
                backoff_limit=0,
                completions=1,
                parallelism=1))
    ...
    def try_create_job(self, k8s_job):
        try:
            ...
            logging.error(
                "Error when calling BatchV1Api.create_namespaced_job(...): %s",
                e)
        ...
        return client.V1Container(
            ...
            volume_mounts=[
                client.V1VolumeMount(
                    name=self.secret_name,
                    mount_path="/var/secrets/evaluation")
            ],
            env=[
                client.V1EnvVar(
...
def csv_to_evaluation_jobs(csv_filename, docker_img, experiment_id, tags):
  ...
  """
    ...
    with open(csv_filename, "rb") as f:
        ...
        eval_jobs = [
            EvaluationJob(docker_img, experiment_id, tags,
                          row["dataset"].strip(),
...
def main(argv):
    ...
    if FLAGS.sweep_file:
        if not FLAGS.parameter_sweep_base_config:
            ...
            return
        jobs_to_monitor = creator.create_jobs(
        ...
        if pod.spec.containers:
            for c in pod.spec.containers:
                if c.name in jobs_to_monitor:
                    logging.info("Job %s signalled event %s, POD Phase: %s",
                                 c.name, event_type, pod.status.phase)
                    if pod.status.phase == "Succeeded":
                        ...
                        num_failed += 1
                        waiting_for_finish -= 1
                    logging.info("Waiting for %d jobs to finish",
                                 waiting_for_finish)
